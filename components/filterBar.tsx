      ...
      <form method="GET" action="/">
        ...
        <Section label="Category">
          <div className="flex flex-col gap-1.5">
            <label
              className={`flex items-center gap-2.5 px-2 py-1.5 rounded-lg cursor-pointer text-sm transition-colors ${
            ...
            {categories.map((item) => (
              ...
              >
                <input
                  ...
                  name="categoryId"
                  value={item.id}
                  defaultChecked={String(item.id) === categoryId}
                  className="accent-indigo-600"
                />
                {item.name}
              </label>
        ...
        <button
          type="submit"
          className="mt-4 w-full rounded-full bg-indigo-500 text-white py-2.5 hover:bg-indigo-600 active:scale-[0.98] transition-all"
        >
          Apply Filters
        </button>
      </form>
